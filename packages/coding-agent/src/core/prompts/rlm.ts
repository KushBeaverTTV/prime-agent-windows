...
const REPL_CONTROL_PROMPT = [
	...
	process.platform === "win32"
		? "This is native Windows. Despite its historical name, `bash()` uses PowerShell by default, not Bash, WSL, or Linux. Use PowerShell syntax, Windows paths, `$env:NAME` for shell environment variables, and `& 'C:\\path with spaces\\program.exe'` for quoted executables. The installed PowerShell may be Windows PowerShell 5.1, where `&&` is unsupported; use separate calls or check exit codes explicitly. Python project virtual environments use `.venv\\Scripts\\python.exe`. An explicitly configured shellPath overrides the default; use that shell's syntax when configured."
		: "Shell commands use the configured POSIX shell. Use paths and commands supported by the host operating system.",
	"",
