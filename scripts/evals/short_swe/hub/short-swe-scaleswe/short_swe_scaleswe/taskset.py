"""short-swe-scaleswe — the fixed 5-task Scale-SWE slice (PrimeIntellect/Scale-SWE-Verified) as a v1 taskset.
...
class ShortSWEScalesweData(vf.TaskData):
...
class ShortSWEScalesweConfig(vf.TasksetConfig):
    """The fixed repository-stratified slice is the package default, so hosted runs
    cannot drift from the published Short SWE identity."""
...
class ShortSWEScalesweTaskset(vf.Taskset[ShortSWEScalesweTask, ShortSWEScalesweConfig]):
    def load(self) -> Iterator[ShortSWEScalesweTask]:
